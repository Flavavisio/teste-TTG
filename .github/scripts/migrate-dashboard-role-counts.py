...
start = app.index('            let countFunc = 0,', fn)
end = app.index('            const licencasAtivas =', start)
old = app[start:end]
...
new = """            const _roleCounts = window.TotalGestDashboardCounts.calculateRoleCounts({
            ...
            const countFunc = _roleCounts.countFunc;
            const countCli = _roleCounts.countCli;
            const osPend = _roleCounts.osPend;
            const osAndamento = _roleCounts.osAndamento;
            const countPonto = _roleCounts.countPonto;
            const countPedidos = _roleCounts.countPedidos;
            const countFolhas = _roleCounts.countFolhas;
            const countReqs = _roleCounts.countReqs;
            const countEncarregados = _roleCounts.countEncarregados;
"""
app = app[:start] + new + app[end:]
assert app.count('TotalGestDashboardCounts.calculateRoleCounts({') == 1

module = """/* Total Gest — contagens do dashboard por perfil. */
(function () {
  'use strict';
...
shell_mod = "    reportsDistributorMetrics: './assets/js/app-reports-distributor-metrics.js',\n"
assert shell.count(shell_mod) == 1
shell = shell.replace(shell_mod, shell_mod + "    dashboardCounts: './assets/js/app-dashboard-counts.js',\n", 1)
...
app = app.replace(init_token, 'reportsDistributorMetrics: true, dashboardCounts: true, syncPrepare: true', 1)

asset_token = "  './assets/js/app-reports-distributor-metrics.js',\n"
assert sw.count(asset_token) == 1
sw = sw.replace(asset_token, asset_token + "  './assets/js/app-dashboard-counts.js',\n", 1)
assert "const CACHE = 'totalgest-v107';" in sw
sw = sw.replace("const CACHE = 'totalgest-v107';", "const CACHE = 'totalgest-v108';", 1)
